...
import 'package:mnema/services/ai_service.dart';

void main() {
  group('AiService Integration Tests', () {
    test('explain returns stream of text with Gemini API call', () async {
...
--------------------''');
    ...
    test('explain returns stream of text with Kimi API call', () async {
      // 1. Get API Key from Environment
      String? apiKey = Platform.environment['KIMI_API_KEY'];
      ...
      if (apiKey == null || apiKey.isEmpty) {
        print('SKIPPING TEST: KIMI_API_KEY not found in environment variables or --dart-define.');
        print('Usage: export KIMI_API_KEY=your_key && flutter test test/ai_service_test.dart');
        return; 
      }

      print('Using Kimi API Key: ${apiKey.substring(0, 4)}****');
